...
public class UserController {
    ...
    public ResponseEntity<ResponseDto<LoginResponseDto<LoginResponse>>> login(
        ...
        if (request.getPayload() != null && !request.getPayload().isBlank()) {
            log.debug("Decrypting RSA-encrypted payload for login");
            try {
                // Decrypt RSA-encrypted payload
                String decryptedJson = RSAUtil.decrypt(request.getPayload(), rsaKeyService.getPrivateKey());
                
                // Parse decrypted JSON to LoginRequest
                LoginRequest decrypted = objectMapper.readValue(decryptedJson, LoginRequest.class);
                request = decrypted;
                log.debug("Successfully decrypted login payload");
            } catch (Exception e) {
                log.error("Failed to decrypt login payload: {}", e.getMessage(), e);
